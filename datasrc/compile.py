...
from datatypes import (
    EmitTypeDeclaration, EmitDefinition
)
...
if gen_network_header:
    ...
    for enum in network.Enums:
        for line in create_enum_table(["%s_%s"%(enum.name, v) for v in enum.values], 'NUM_%sS'%enum.name): 
            print(line)
        print("")

    for flag in network.Flags:
        for line in create_flags_table(["%s_%s" % (flag.name, v) for v in flag.values]): 
            print(line)
        print("")
