...
class Struct(BaseType):
    ...
    def Members(self):
        ...
        try:
            ...
            print(f"Error while sorting members in Struct '{self._type_name}': {e}")
            for v in m:
                print(v.name, v.var)
            raise
        return m
    ...
    def EmitPreDefinition(self, target_name):
        super().EmitPreDefinition(target_name)
        lines = []
        for member in self.Members():
...
class Array(BaseType):
	def __init__(self, type):
		self.type = type
		self.items = []
		super().__init__(type.TypeName())
	def Add(self, instance):
		if instance.TypeName() != self.type.TypeName():
			raise TypeError("Type mismatch: expected %s, got %s" % (self.type.TypeName(), instance.TypeName()))
...
class Pointer(BaseType):
    def __init__(self, type, target):
        self.target = target
        super().__init__("%s*"%type().TypeName())
    def Set(self, target):
        self.target = target
    def EmitDefinition(self, name):
...
class TextureHandle(BaseType):
    def __init__(self):
        super().__init__("IGraphics::CTextureHandle")
    def EmitDefinition(self, name):
        return ["IGraphics::CTextureHandle()"]	

class SampleHandle(BaseType):
    def __init__(self):
        super().__init__("ISound::CSampleHandle")
    def EmitDefinition(self, name):
...
class NetEvent(NetObject):
    def __init__(self, name, variables, ex=None):
        super().__init__(name, variables, ex=ex)
...
class NetMessage(NetObject):
    def __init__(self, name, variables, ex=None, teehistorian=True):
        super().__init__(name, variables, ex=ex, fixup=False)
...
class NetObjectEx(NetObject):
    def __init__(self, name, ex, variables, fixup=True):
        super().__init__(name, variables, ex=ex, fixup=fixup)
...
class NetMessageEx(NetMessage):
    def __init__(self, name, ex, variables, teehistorian=True):
        super().__init__(name, variables, ex=ex, teehistorian=teehistorian)
...
class NetEnum(NetIntRange):
    def __init__(self, name, enum):
        super().__init__(name, 0, len(enum.values)-1)
...
class NetBool(NetIntRange):
    def __init__(self, name, default=None):
        ...
        super().__init__(name, 0, 1, default=default)

class NetTick(NetIntRange):
    def __init__(self, name):
        super().__init__(name, 0, 'max_int')
