...
public:
	...
	*/
	...
	int SendPackMsgTranslate(CNetMsg_Sv_Chat *pMsg, int Flags, int ClientID)
	{
		...
		if (pMsg->m_Mode != CHAT_WHISPER && pMsg->m_ClientID >= 0 && ((!(Flags&MSGFLAG_NO_TRANSLATE) && pMsg->m_Mode == CHAT_TEAM && pMsg->m_ClientID != ClientID) || !Translate(pMsg->m_ClientID, ClientID)))
		{
