...
void CConfigManager::Reset()
{
	...
	#define MACRO_CONFIG_UTF8STR(Name,ScriptName,size,len,def,flags,desc,accesslevel) str_utf8_copy_num(m_Values.m_##Name, def, size, len);
...
void CConfigManager::RestoreStrings()
{
	...
	#define MACRO_CONFIG_UTF8STR(Name,ScriptName,size,len,def,flags,desc,accesslevel) if(!m_Values.m_##Name[0] && def[0]) str_utf8_copy_num(m_Values.m_##Name, def, size, len);
