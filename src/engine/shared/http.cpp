...
#pragma GCC diagnostic pop
	...
	curl_easy_setopt(pH, CURLOPT_USERAGENT, GAME_NAME " " GAME_VERSION " (" CONF_PLATFORM_STRING "; " CONF_ARCH_STRING ")");
...
#pragma GCC diagnostic pop
	...
	if(m_pConfig->m_Bindaddr[0] != '\0')
	{
		curl_easy_setopt(pH, CURLOPT_INTERFACE, m_pConfig->m_Bindaddr);
	}
...
void CHttpRequest::OnCompletionInternal(void *pHandle, unsigned int Result)
{
	...
	if(Code != CURLE_OK)
	{
		if(m_pConfig->m_DbgCurl || m_LogProgress >= HTTPLOG::FAILURE)
		{
	...
	{
		if(m_pConfig->m_DbgCurl || m_LogProgress >= HTTPLOG::ALL)
		{
...
void CHttpRequest::WriteToFile(IStorage *pStorage, const char *pDest, int StorageType)
{
	...
	str_format(m_aDestAbsoluteTmp, sizeof(m_aDestAbsoluteTmp), "%s.%d.tmp", m_aDestAbsolute, pid());
}
