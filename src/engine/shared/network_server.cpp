	...
	int MaxClients, int MaxClientsPerIP, NETFUNC_NEWCLIENT pfnNewClient, NETFUNC_DELCLIENT pfnDelClient, void *pUser)
{
	...
	NETSOCKET Socket2 = net_udp_create(BindAddr, 0);
	if(!Socket.type)
		return false;

	// init
	m_pNetBan = pNetBan;
	Init(Socket, Socket2, pConfig, pConsole, pEngine);
...
*/
int CNetServer::Recv(CNetChunk *pChunk, TOKEN *pResponseToken, bool *pSevendown, bool Two)
{
	while(1)
	{
		// check for a chunk
		...
		NETADDR Addr;
		int Result = UnpackPacket(&Addr, m_RecvUnpacker.m_aBuffer, &m_RecvUnpacker.m_Data, pSevendown, Two, this);
		...
		if(!Result)
		{
			...
			if(NetBan() && NetBan()->IsBanned(&Addr, aBuf, sizeof(aBuf), &LastInfoQuery))
			{
				...
				{
					SendControlMsg(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, 0, NET_CTRLMSG_CLOSE, aBuf, str_length(aBuf) + 1, *pSevendown, Two, NET_SECURITY_TOKEN_UNSUPPORTED);
				}
				continue;
			}
			...
			for(int i = 0; i < NET_MAX_CLIENTS; i++)
			{
				...
				{
					if(m_aSlots[i].m_Connection.Feed(&m_RecvUnpacker.m_Data, &Addr, *pSevendown, Two))
					{
			...
			if (!*pSevendown)
			{
				int Accept = m_TokenManager.ProcessMessage(&Addr, &m_RecvUnpacker.m_Data, Two);
				if(Accept <= 0)
					continue;

				if(ControlMsg == NET_CTRLMSG_TOKEN)
					m_TokenCache.AddToken(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, NET_TOKENFLAG_RESPONSEONLY, Two);
				else if(ControlMsg == NET_CTRLMSG_CONNECT)
				{
					SendControlMsg(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, 0, NET_CTRLMSG_ACCEPT, 0, 0, false, Two);
					AcceptConnect = true;
				}
			}
			else
			{
				...
				if(ControlMsg == NET_CTRLMSG_CONNECT)
				{
					...
					SendControlMsg(&Addr, 0, 0, NET_CTRLMSG_ACCEPT, SECURITY_TOKEN_MAGIC, sizeof(SECURITY_TOKEN_MAGIC), true, Two, SecurityToken);
				}
			...
			if (m_RecvUnpacker.m_Data.m_Flags&NET_PACKETFLAG_CONTROL)
			{
				if(AcceptConnect)
				{
					if (Connlimit(Addr))
					{
						...
						SendControlMsg(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, 0, NET_CTRLMSG_CLOSE, LimitMsg, sizeof(LimitMsg), *pSevendown, Two, SecurityToken);
					...
					if(m_NumClients >= m_MaxClients)
					{
						...
						SendControlMsg(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, 0, NET_CTRLMSG_CLOSE, FullMsg, sizeof(FullMsg), *pSevendown, Two, SecurityToken);
						continue;
					}
					...
					if(FoundAddr >= MaxClientsPerIP)
					{
						...
						SendControlMsg(&Addr, m_RecvUnpacker.m_Data.m_ResponseToken, 0, NET_CTRLMSG_CLOSE, aBuf, str_length(aBuf) + 1, *pSevendown, Two, SecurityToken);
						continue;
					}
					...
					for(int i = 0; i < NET_MAX_CLIENTS; i++)
					{
						...
						{
							m_NumClients++;
							m_aSlots[i].m_Connection.DirectInit(&Addr, &m_RecvUnpacker.m_Data, SecurityToken, *pSevendown, Two);
							if(m_pfnNewClient)
								m_pfnNewClient(i, *pSevendown, Two, m_UserPtr);
...
int CNetServer::Send(CNetChunk *pChunk, TOKEN Token, bool Sevendown, bool Two)
{
	if(pChunk->m_Flags&NETSENDFLAG_CONNLESS)
	{
		...
		{
			SendPacketConnless(&pChunk->m_Address, Token, m_TokenManager.GenerateToken(&pChunk->m_Address), pChunk->m_pData, pChunk->m_DataSize, Sevendown, Two);
		}
		else
		{
			if(pChunk->m_ClientID == -1)
			{
				m_TokenCache.SendPacketConnless(&pChunk->m_Address, pChunk->m_pData, pChunk->m_DataSize, Two);
			}
			else
			{
