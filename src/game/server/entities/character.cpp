		...
		switch (GetActiveWeapon())
		{
			case WEAPON_HAMMER:
			{
				...
				for (int i = 0; i < Num; ++i)
				{
					...
					{
						pProj->HitProjectile(this, Direction, MouseTarget);
...
void CCharacter::HandleWeapons()
{
	...
	// ammo regen
	int AmmoRegenTime = g_pData->m_Weapons.m_aId[GameServer()->GetWeaponType(GetActiveWeapon())].m_Ammoregentime;
...
bool CCharacter::TakeDamage(vec2 Force, vec2 Source, int Dmg, int From, int Weapon)
{
	...
	if (SetKiller && From >= 0 && GameServer()->m_apPlayers[From] && From != m_pPlayer->GetCID())
	{
...
void CCharacter::HandleTiles(int Index)
{
	...
	if (!FightStarted && !GameServer()->Durak()->InDurakGame(m_pPlayer->GetCID()))
	{
		...
		else if (!m_GotMoneyXPBomb && (m_TileIndex == TILE_MONEY_XP_BOMB || m_TileFIndex == TILE_MONEY_XP_BOMB))
		{
			...
			else if (m_pPlayer->m_LastMoneyXPBomb < Server()->Tick() - Server()->TickSpeed() * Config()->m_SvMoneyXpBombDelay)
			{
