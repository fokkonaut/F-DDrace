...
int CCharacter::NetworkClipped(int SnappingClient, vec2 CheckPos)
{
	...
	if(absolute(dy) > GameServer()->m_apPlayers[SnappingClient]->m_ShowDistance.x)
		return 1;
...
void CCharacter::DropWeapon(int WeaponID, bool OnDeath, float Dir)
{
	...
	if ((m_FreezeTime && !OnDeath) || !Config()->m_SvDropWeapons || Config()->m_SvMaxWeaponDrops == 0 || !m_aWeapons[WeaponID].m_Got
		|| (WeaponID == WEAPON_NINJA && !m_ScrollNinja) || WeaponID == WEAPON_TASER || WeaponID == WEAPON_PORTAL_RIFLE)
		return;
