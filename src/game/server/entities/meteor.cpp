...
void CMeteor::Tick()
{
	...
	float Friction = GameServer()->TuningFromChrOrZone(m_Owner, m_TuneZone)->m_MeteorFriction / 1000000.f;
	float MaxAccel = GameServer()->TuningFromChrOrZone(m_Owner, m_TuneZone)->m_MeteorMaxAccel / 1000.f;
	float AccelPreserve = GameServer()->TuningFromChrOrZone(m_Owner, m_TuneZone)->m_MeteorAccelPreserve / 1000.f;
