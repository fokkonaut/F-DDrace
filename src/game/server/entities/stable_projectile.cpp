...
bool CStableProjectile::FillExtraInfo(CNetObj_DDNetProjectile *pProj)
{
	...
	float Angle = -atan2f(m_VelX, m_VelY);
	...
	pProj->m_X = round_to_int(m_LastResetPos.x * 100.0f);
	pProj->m_Y = round_to_int(m_LastResetPos.y * 100.0f);
	pProj->m_Angle = round_to_int(Angle * 1000000.0f);
	pProj->m_Data = Data;
	pProj->m_StartTick = m_LastResetTick;
	pProj->m_Type = m_Type;
	return true;
