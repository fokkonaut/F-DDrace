...
void CGameWorld::UpdatePlayerMaps()
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		for (int j = 0; j < MAX_CLIENTS; j++)
		{
			...
			// set distance for same ip players to very far away, so they dont get added to the normal algorithm
		...
		for (int j = 0; j < VANILLA_MAX_CLIENTS; j++)
		{
			...
			if (Dist[pMap[j]].first > 5e9 && !GameServer()->m_apPlayers[i]->m_aSameIP[pMap[j]]) pMap[j] = -1;
			else rMap[pMap[j]] = j;
		}
		...
		for (int j = 0; j < MAX_CLIENTS; j++)
		{
			if (GameServer()->m_apPlayers[i]->m_aSameIP[j])
			{
				...
				if (pMap[GameServer()->m_apPlayers[j]->m_FakeID] == j)
					rMap[j] = GameServer()->m_apPlayers[j]->m_FakeID;
				pMap[GameServer()->m_apPlayers[j]->m_FakeID] = j;
			}
		}
