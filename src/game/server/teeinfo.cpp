...
static Skin s_SkinsSevendown[NUM_SEVENDOWN_SKINS] = {
...
{"warpaint",{"standard","warpaint","","standard","standard","standard"},{1,0,0,1,1,0},{1944919,-16711808,65408,750337,1944919,65408}},
{"greensward",{"greensward","duodonny","","standard","standard","standard"},{1,1,0,0,0,0},{5635840,-11141356,65408,65408,65408,65408}},
{"greyfox2",{"greyfox_body","greyfox_marking","hair","standard","standard","standard"},{1,1,1,1,1,1},{1245401,1224736916,52,7864575,32,0}},
{"spooky_ghost",{"spiky","tricircular","","standard","standard","colorable"},{1,1,1,1,1,1},{255,-16777016,255,184,9765959,255}},
...
{"cavebat",{"bat","belly2","","standard","standard","colorable"},{1,1,0,1,1,1},{1091513,-1744165075,65408,666952,949148,553007}},
{"force",{"force","wildpaint","","standard","standard","standard"},{1,1,0,1,1,1},{1577780,907949571,65408,1769535,1835008,0}},
{"fox",{"fox","fox","","standard","standard","colorable"},{1,1,0,1,1,1},{1102443,-485425166,65408,1094755,1102450,1441632}},
{"greycoon",{"raccoon","coonfluff","","standard","standard","standard"},{1,1,0,1,1,1},{917651,-15269783,65408,1769643,1245336,1085234}},
{"greyfox",{"fox","cammostripes","","standard","standard","colorable"},{1,1,0,1,1,1},{1051203,538383807,65408,10230803,2100280,1858113}},
...
{"warmouse",{"mouse","mice","","standard","standard","negative"},{1,1,0,1,1,1},{1835221,-16776961,65408,1769727,1869823,51228}},
...
void CTeeInfo::FromSevendown()
{
	...
	for(int p = 0; p < NUM_SKINPARTS; p++)
	{
		...
		m_aUseCustomColors[p] = s_SkinsSevendown[SKIN_DEFAULT].m_aUseCustomColors[p];
		m_aSkinPartColors[p] = s_SkinsSevendown[SKIN_DEFAULT].m_aSkinPartColors[p];
	}
	...
	for(int i = 0; i < NUM_SEVENDOWN_SKINS; i++)
	{
		...
		if(!str_comp(m_Sevendown.m_SkinName, s_SkinsSevendown[i].m_SkinName))
		{
			for(int p = 0; p < NUM_SKINPARTS; p++)
			{
				str_copy(m_aaSkinPartNames[p], s_SkinsSevendown[i].m_apSkinPartNames[p], 24);
				m_aUseCustomColors[p] = s_SkinsSevendown[i].m_aUseCustomColors[p];
				m_aSkinPartColors[p] = s_SkinsSevendown[i].m_aSkinPartColors[p];
			}
			Match = i;
			break;
		}
...
{
	// reset to default skin
	str_copy(m_Sevendown.m_SkinName, "default", sizeof(m_Sevendown.m_SkinName));
	...
	for(int s = 0; s < NUM_SEVENDOWN_SKINS; s++)
	{
		...
		for(int p = 0; p < NUM_SKINPARTS; p++)
		{
			if(str_comp(m_aaSkinPartNames[p], s_SkinsSevendown[s].m_apSkinPartNames[p])
					|| m_aUseCustomColors[p] != s_SkinsSevendown[s].m_aUseCustomColors[p]
					|| (m_aUseCustomColors[p] && m_aSkinPartColors[p] != s_SkinsSevendown[s].m_aSkinPartColors[p]))
			{
				match = false;
				break;
			}
		...
		if(match)
		{
			str_copy(m_Sevendown.m_SkinName, s_SkinsSevendown[s].m_SkinName, sizeof(m_Sevendown.m_SkinName));
			return;
		}
	}
	...
	for(int s = 0; s < NUM_SEVENDOWN_SKINS; s++)
	{
		...
		int matches = 0;
		for(int p = 0; p < 3; p++)
			if(str_comp(m_aaSkinPartNames[p], s_SkinsSevendown[s].m_apSkinPartNames[p]) == 0)
				matches++;
	...
	str_copy(m_Sevendown.m_SkinName, s_SkinsSevendown[best_skin].m_SkinName, sizeof(m_Sevendown.m_SkinName));
	...
	if (m_aSkinPartColors[SKINPART_BODY] == s_SkinsSevendown[best_skin].m_aSkinPartColors[SKINPART_BODY])
		CustomColors = false;
