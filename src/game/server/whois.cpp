...
int CWhoIs::Init(CGameContext *pGameServer)
{
	...
	if (!GameServer()->Config()->m_SvWhoIsFile[0])
        return -1;
	...
	if ((src_fd = open(GameServer()->Config()->m_SvWhoIsFile, O_RDONLY, 0777)) < 0) {
...
void CWhoIs::OnClientEnter(int ClientID)
{
    ...
    if (GameServer()->Config()->m_SvWhoIs) {
        if ((fd = open(GameServer()->Config()->m_SvWhoIsFile, O_RDWR|O_CREAT|O_APPEND, 0777)) < 0)
            perror("open");
        else
